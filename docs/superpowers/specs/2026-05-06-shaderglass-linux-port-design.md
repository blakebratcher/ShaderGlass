...
- Bring ShaderGlass's shader-effect overlay to Linux on both X11 and Wayland.
...
- Wayland transparent overlay. Wayland's security model intentionally precludes it; on Wayland, ShaderGlass is a viewer window. X11 keeps the always-on-top transparent overlay.
...
ShaderGlassLinux/             # NEW
  src/
    ...
    util/
      Config.{h,cpp}            # ~/.config/shaderglass/
      Logging.{h,cpp}
  CMakeLists.txt
External/                     # existing
CMakeLists.txt                # NEW top-level — builds ShaderGC + ShaderGlassLinux
```
...
The Windows app embeds precompiled **HLSL bytecode** in `ShaderGlass/Shaders/RetroArch/*.h` (generated by ShaderGen at build time). That bytecode is unusable on Linux. Three options were considered:
...
The Linux app **does not embed any precompiled shaders**. Instead it ships the raw `.slangp`, `.slang`, and texture files in `~/.local/share/shaderglass/shaders/` (or compiled-in via a CMake-generated resource for the AppImage build), and compiles each preset on first selection. Compiled SPIR-V is cached on disk under `~/.cache/shaderglass/spirv/` keyed by content hash (`ShaderCache.cpp` already does this on Windows for the bytecode, the same logic applies). Subsequent selections of the same preset hit the cache and pipeline-build is sub-millisecond.
...
ImGui state (parameters, dock layout) persists to `~/.config/shaderglass/imgui.ini` and `~/.config/shaderglass/config.json`.

### Display-server selection

At startup:
```
if getenv("WAYLAND_DISPLAY") and not getenv("SHADERGLASS_FORCE_X11"):
...
XWayland is not auto-detected — under XWayland we'd use the X11 backend, which works but loses Wayland-native capture. An env override (`SHADERGLASS_FORCE_WAYLAND=1`) handles edge cases.
...
- Output: a single `shaderglass` binary + `~/.local/share/shaderglass/` data dir for any runtime assets we end up shipping.
...
- **One renderer (Vulkan), two backends per axis (capture, output)** — minimum viable abstraction. Adding OpenGL fallback is a hypothetical we can resist; Vulkan is broadly available on every GPU that runs ShaderGlass on Windows.
- **Share `ShaderGC/`, not `ShaderGlass/`** — the shader compiler is the high-leverage shared code; the windowing/capture/render is genuinely different per platform and abstracting it would be a yak shave.
