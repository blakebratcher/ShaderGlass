# ShaderScope Linux M2 — Wayland Capture Implementation Plan
...
**Reference spec:** `docs/superpowers/specs/2026-05-06-shaderscope-linux-m2-wayland-capture-design.md`
...
**Files:**
- Create: `ShaderScope/src/capture/WaylandCaptureSession.h`
- Modify: `ShaderScope/src/capture/CapturedFrame.h`
...
```bash
git add ShaderScope/src/capture/{WaylandCaptureSession.h,CapturedFrame.h}
...
**Files:**
- Create: `ShaderScope/src/capture/FakeWaylandCaptureSession.h`
- Create: `ShaderScope/src/capture/FakeWaylandCaptureSession.cpp`
- Modify: `ShaderScope/CMakeLists.txt` — add the new source
...
Add to `ShaderScope/CMakeLists.txt` `shaderscope_core` source list:
```cmake
...
- [ ] **Step 2.5: Commit**
```bash
git add ShaderScope/{CMakeLists.txt,src/capture/FakeWaylandCaptureSession.{h,cpp}}
...
**Files:**
- Create: `ShaderScope/src/capture/WaylandCapture.h`
- Create: `ShaderScope/src/capture/WaylandCapture.cpp`
- Create: `ShaderScope/tests/test_wayland_capture_with_fake_session.cpp`
- Modify: `ShaderScope/CMakeLists.txt`, `ShaderScope/tests/CMakeLists.txt`
- Create: `ShaderScope/tests/data/reference_fake_session_4x4.png` (committed; generated by running the test once)
...
`ShaderScope/tests/test_wayland_capture_with_fake_session.cpp`:
```cpp
...
TEST(WaylandCaptureWithFakeSession, RendersSyntheticFrameToReferencePng) {
    ...
    fs::path tmp = fs::temp_directory_path() / "shaderscope_fake_session_out.png";
    fs::remove(tmp);
...
target_link_libraries(wayland_capture_fake_tests PRIVATE shaderscope_core gtest_main)
...
```bash
cp ShaderScope/tests/data/reference_passthrough_4x4.png \
   ShaderScope/tests/data/reference_fake_session_4x4.png
```
...
```bash
git add ShaderScope/{CMakeLists.txt,src/capture/WaylandCapture.{h,cpp},tests/{CMakeLists.txt,test_wayland_capture_with_fake_session.cpp,data/reference_fake_session_4x4.png}}
...
- [ ] **Step 4.2: Modify `ShaderScope/CMakeLists.txt`**
...
In the `target_link_libraries(shaderscope_core PUBLIC ...)` block append `PkgConfig::DBUS PkgConfig::PIPEWIRE`. The libdrm headers are needed only at preprocess time (`drm_fourcc.h` constants) — add to includes:
```cmake
target_include_directories(shaderscope_core PRIVATE ${LIBDRM_INCLUDE_DIRS})
```
...
**Files:**
- Create: `ShaderScope/src/capture/PortalCaptureSession.h`
- Create: `ShaderScope/src/capture/PortalCaptureSession.cpp`
- Modify: `ShaderScope/CMakeLists.txt`
...
In `ShaderScope/CMakeLists.txt` `shaderscope_core` source list:
```cmake
...
- [ ] **Step 5.5: Commit**
```bash
git add ShaderScope/{CMakeLists.txt,src/capture/PortalCaptureSession.{h,cpp}}
...
**Files:**
- Modify: `ShaderScope/src/capture/PortalCaptureSession.cpp`
- Modify: `ShaderScope/src/main.cpp` — add a `--debug-portal` flag
...
- [ ] **Step 6.5: Commit**
```bash
git add ShaderScope/{src/capture/PortalCaptureSession.cpp,src/main.cpp}
...
**Files:**
- Create: `ShaderScope/src/util/XdgConfig.h`
- Create: `ShaderScope/src/util/XdgConfig.cpp`
- Create: `ShaderScope/tests/test_xdg_config.cpp`
- Modify: `ShaderScope/src/capture/PortalCaptureSession.cpp` — read/write the token
- Modify: `ShaderScope/CMakeLists.txt`, `ShaderScope/tests/CMakeLists.txt`

`XdgConfig` is a small helper that resolves `${XDG_CONFIG_HOME:-$HOME/.config}/shaderscope/<name>` and reads/writes a single-line text value with mode 0600.

- [ ] **Step 7.1: Failing test**

`ShaderScope/tests/test_xdg_config.cpp`:
```cpp
#include <gtest/gtest.h>
...
target_link_libraries(xdg_config_tests PRIVATE shaderscope_core gtest_main)
...
static fs::path baseDir() {
    ...
    if (xdg && *xdg) return fs::path(xdg) / "shaderscope";
    const char* home = std::getenv("HOME");
    if (!home) throw std::runtime_error("XdgConfig: HOME not set");
    return fs::path(home) / ".config" / "shaderscope";
}
...
Add `src/util/XdgConfig.cpp` to `shaderscope_core` source list.
...
If the second run still shows the picker, inspect `~/.config/shaderscope/portal-token` — should contain a non-empty single line.

- [ ] **Step 7.8: Commit**
```bash
git add ShaderScope/{CMakeLists.txt,src/util/XdgConfig.{h,cpp},src/capture/PortalCaptureSession.cpp,tests/{CMakeLists.txt,test_xdg_config.cpp}}
git commit -m "feat(capture): persist portal restore token to ~/.config/shaderscope/"
```
...
void PortalCaptureSession::initPipeWire() {
    ...
    m_pwLoop = pw_thread_loop_new("shaderscope-pw", nullptr);
    ...
    auto* props = pw_properties_new(
        ...
        nullptr);
    m_pwStream = pw_stream_new(m_pwCore, "shaderscope-capture", props);
    if (!m_pwStream) {
        pw_thread_loop_unlock(m_pwLoop);
...
**Files:**
- Create: `ShaderScope/src/render/DmaBufImport.h`
- Create: `ShaderScope/src/render/DmaBufImport.cpp`
- Modify: `ShaderScope/CMakeLists.txt` — add the new source
...
Add `src/render/DmaBufImport.cpp` to `shaderscope_core` source list.
...
- [ ] **Step 9.4: Commit**
```bash
git add ShaderScope/{CMakeLists.txt,src/render/DmaBufImport.{h,cpp}}
...
**Files:**
- Create: `ShaderScope/tests/test_dmabuf_import.cpp`
...
In `ShaderScope/tests/CMakeLists.txt`, before the gbm test target:
```cmake
...
`ShaderScope/tests/test_dmabuf_import.cpp`:
```cpp
...
In `ShaderScope/tests/CMakeLists.txt`:
```cmake
add_executable(dmabuf_import_tests test_dmabuf_import.cpp)
target_link_libraries(dmabuf_import_tests PRIVATE shaderscope_core gtest_main)
if(GBM_FOUND)
...
- [ ] **Step 10.5: Commit**
```bash
git add ShaderScope/tests/{CMakeLists.txt,test_dmabuf_import.cpp}
...
**Files:**
- Modify: `ShaderScope/src/capture/PortalCaptureSession.cpp`
- Modify: `ShaderScope/src/capture/PortalCaptureSession.h` — store an `ImportedDmaBuf` cache
- Modify: `ShaderScope/src/capture/CapturedFrame.h` — add a `vkImage`/`vkView` field for DmaBuf frames
- Modify: `ShaderScope/src/main.cpp` — handle the DmaBuf path in the windowed render loop
...
m_useDmaBuf = (m_vkCtx && DmaBufImport::isSupported(*m_vkCtx)
               && std::getenv("SHADERSCOPE_DISABLE_DMABUF") == nullptr);
...
- [ ] **Step 11.7: Commit**
```bash
git add ShaderScope/{src/capture/{PortalCaptureSession.{h,cpp},CapturedFrame.h},src/render/{DmaBufImport.cpp,RenderEngine.{h,cpp},ShaderPipeline.{h,cpp}},src/main.cpp}
...
```markdown
# Building ShaderScope on Linux (M2 status)
...
## Run
- `./build/ShaderScope/shaderscope <some.png>` — windowed passthrough on a static image (M1).
- `./build/ShaderScope/shaderscope --capture wayland-screen` — capture a screen / window via the portal (M2).
- `./build/ShaderScope/shaderscope --debug-portal` — exercise just the portal handshake.

## Status
- M1: ✅ shipped
- M2: 🚧 in progress / shipped (this milestone)
- M3: X11 capture — pending
- M4+: see `docs/superpowers/specs/2026-05-06-shaderscope-linux-port-design.md`
```
...
3. `./build/ShaderScope/shaderscope --debug-portal` opens the KDE picker, walks the handshake, prints the PipeWire fd + node id, and exits cleanly.
4. Re-running `--debug-portal` skips the picker (restore token works).
5. `./build/ShaderScope/shaderscope --capture wayland-screen` opens a window that displays whichever source the user picks in the portal, rendered through the passthrough shader.
6. `SHADERSCOPE_DISABLE_DMABUF=1 ./build/ShaderScope/shaderscope --capture wayland-screen` does the same via the CPU path. Visually identical output.
...
- **`SHADERSCOPE_DISABLE_DMABUF=1` env var.** Easy debug toggle; surfaces the CPU path on demand without a code change.
